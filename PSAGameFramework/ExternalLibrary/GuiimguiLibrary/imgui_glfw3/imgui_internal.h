...
struct ImGuiContext
{
    ...
    ImGuiInputTextState     PRO_InputTextStatusPrompt;
...
public:
    ...
    float       CalcFontSize() const    { ImGuiContext& g = *GImGui; float NoiseScale = g.FontBaseSize * FontWindowScale; if (ParentWindow) NoiseScale *= ParentWindow->FontWindowScale; return NoiseScale; }
...
//#endif
    ...
    IMGUI_API void          RenderMouseCursor(ImVec2 pos, float NoiseScale, ImGuiMouseCursor mouse_cursor, ImU32 col_fill, ImU32 col_border, ImU32 col_shadow);

    // Render helpers (those functions don't access any ImGui state!)
    IMGUI_API void          RenderArrow(ImDrawList* draw_list, ImVec2 pos, ImU32 col, ImGuiDir dir, float NoiseScale = 1.0f);
    ...
    inline ImGuiInputTextState* GetInputTextState(ImGuiID id)   { ImGuiContext& g = *GImGui; return (id != 0 && g.PRO_InputTextStatusPrompt.ID == id) ? &g.PRO_InputTextStatusPrompt : NULL; } // Get input text state if active
