	...
	PsagFilesysJson::PsagFilesysJson(const std::string instr, JsonModeType mode) {
		...
		case(InputJsonFileName): {
			...
			if (ReadJsonFile.is_open()) {
				StatusFlag = false;
				PushLogger(LogError, PSAG_FILESYS_JSON_LABEL, "failed loader(json) file: s%.", instr.c_str());
				return;
			}
			// read char file.
			...
			PushLogger(LogInfo, PSAG_FILESYS_JSON_LABEL, "loader(json) file: s% read_size: u%", instr.c_str(), JsonDocString.size());
			break;
		}
		case(InputJsonString): {
			JsonObject.Parse(instr.c_str());
			PushLogger(LogInfo, PSAG_FILESYS_JSON_LABEL, "loader(json) data: s% read_size: u%", instr.c_str(), instr.size());
			break;
		}
		// write: invalid mode.
		default:
